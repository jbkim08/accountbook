...
public class TransactionController {
    ...
    public String index(
        ...
        model.addAttribute("transactions", transactionService.getTransactions(page, pageSize, startDate, endDate, category, username));
        int totalPages = transactionService.getTotalPages(pageSize, startDate, endDate, category, username);
        ...
        model.addAttribute("totalExpense", transactionService.getTotalExpense(username));
        return "index";
    }
    // id로 해당 트랜젝션을 삭제한다.
    @GetMapping("/delete/{id}")
    public String deleteTransaction(@PathVariable Long id, @AuthenticationPrincipal User user) {
        transactionService.removeTransaction(id, user.getUsername());
        return "redirect:/";
    }
    // 새 트랜젝션 저장 처리
    @PostMapping("/add")
    public String addTransaction(@Valid Transaction transaction,
        ...
        if (bindingResult.hasErrors()) {
            ...
            model.addAttribute("transactions", transactionService.getAllTransactions(user.getUsername()));
            model.addAttribute("totalIncome", transactionService.getTotalIncome(user.getUsername()));
            model.addAttribute("totalExpense", transactionService.getTotalExpense(user.getUsername()));
            return "index"; //되돌아 가기
        }
